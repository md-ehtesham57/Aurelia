...
const Header = () => {
  ...
  // Subtle elevation once the page scrolls — plain state toggle, no animation library needed
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-fluid-4">
        {/* 3-column grid keeps the logo genuinely centered regardless of side content width */}
          ...
          <div className="flex items-center gap-fluid-6">
            ...
            <nav className="hidden lg:flex items-center gap-fluid-6">
              <div className="group relative">
                <button className="flex items-center gap-1 text-sm text-text-muted hover:text-text transition-colors cursor-default">
                  Shop
      ...
      <div
        className={`lg:hidden border-t transition-all duration-300 overflow-hidden
                    ${mobileMenuOpen ? 'max-h-72 opacity-100 border-surface' : 'max-h-0 opacity-0 border-transparent'}`}
      >
        <nav className="px-fluid-4 py-fluid-4 space-y-1">
          <Link
            to="/products"
